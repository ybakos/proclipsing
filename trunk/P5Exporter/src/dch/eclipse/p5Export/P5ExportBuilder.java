...
{ 
  ...
  private static String SLASH, MAIN_PATT= 
    "(?:.*)static[^v]+void\\s+main\\s*\\(\\s*String[^\\[]*\\[\\][^\\]]*[^\\)]*\\).*";
  ...
  {    
if(DBUG)System.out.println("P5ExportBuilder.P5ExportBuilder()");
...
if(DBUG)System.out.println("Checking lib dir ----------------------------------------");

    File libDir = new File(System.getProperty("user.dir"), "lib");
    if (libDir.exists()) 
    {

if(DBUG)System.out.println("Found lib dir... checking prefs file");
...
if(DBUG)System.out.println("No prefs file... lets delete and recreate");
        P5ExportUtils.deleteDir(libDir);
if(DBUG)System.out.println("deleted lib dir...");
        libDir.mkdir();
if(DBUG)System.out.println("created lib dir...");
        P5ExportUtils.writeFile(verFile, "p5e.version="+P5ExportPlugin.VERSION);
if(DBUG)System.out.println("writing file..."+verFile);
      }
    }
  ...
  {    
    mainClass = pConfiguration.getAttribute
      (IJavaLaunchConfigurationConstants.ATTR_MAIN_TYPE_NAME,"");    
    
    srcPath = pConfiguration.getAttribute 
      (IJavaLaunchConfigurationConstants.ATTR_DEFAULT_SOURCE_PATH,"");   
    
    srcPath = pConfiguration.getAttribute
      (IJavaLaunchConfigurationConstants.ATTR_SOURCE_PATH, srcPath);
    
    if (srcPath.length()==0) srcPath = projectLocation+SLASH+"src"+SLASH;
 
    String vmArgsAttr = pConfiguration.getAttribute
      (IJavaLaunchConfigurationConstants.ATTR_VM_ARGUMENTS,"");
    if (vmArgsAttr != null) vmArgs = vmArgsAttr;
     
    String progArgsAttr = pConfiguration.getAttribute
      (IJavaLaunchConfigurationConstants.ATTR_PROGRAM_ARGUMENTS,"");
    if (progArgsAttr != null) progArgs = progArgsAttr;
